...
function createTabs() {
  ...
  testTemplates.forEach((template, index) => {
    ...
    button.className = `px-4 py-2.5 rounded-lg text-sm font-medium cursor-pointer transition-all whitespace-nowrap shadow-sm ${
      index === 0 
        ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-md hover:shadow-lg transform hover:scale-105' 
        : 'bg-white text-gray-700 border border-gray-300 hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 hover:border-blue-400 hover:shadow-md transform hover:scale-105'
    }`;
...
function selectTemplate(template: TestTemplate) {
  ...
  buttons.forEach((btn, index) => {
    if (testTemplates[index].id === template.id) {
      btn.className = 'px-4 py-2.5 rounded-lg text-sm font-medium cursor-pointer transition-all whitespace-nowrap shadow-sm bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-md hover:shadow-lg transform hover:scale-105';
    } else {
      btn.className = 'px-4 py-2.5 rounded-lg text-sm font-medium cursor-pointer transition-all whitespace-nowrap shadow-sm bg-white text-gray-700 border border-gray-300 hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 hover:border-blue-400 hover:shadow-md transform hover:scale-105';
    }
  });

  // 更新描述和代码
  if (descriptionEl) {
    const descContent = descriptionEl.querySelector('div');
    if (descContent) {
      descContent.innerHTML = `<strong class="block mb-2 text-gray-800 font-bold text-base">${template.name}</strong><span class="text-gray-600">${template.description}</span>`;
    }
  }
  ...
  if (preview) {
    preview.innerHTML = `
      <div class="flex flex-col items-center justify-center h-full text-gray-500">
        ...
        <div class="text-center italic">代码已加载，点击"渲染"按钮查看效果</div>
      </div>
    `;
  }
}
...
function updateStatus(type: 'success' | 'error' | 'pending', text: string) {
  ...
  const statusConfig = {
    success: { 
      class: 'bg-green-100 text-green-800 shadow-sm',
      icon: '✅'
    },
    error: { 
      class: 'bg-red-100 text-red-800 shadow-sm',
      icon: '❌'
    },
    pending: { 
      class: 'bg-yellow-100 text-yellow-800 shadow-sm',
      icon: '⏳'
    }
  };
  
  const config = statusConfig[type];
  statusBadge.className = `inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${config.class}`;
  statusBadge.textContent = `${config.icon} ${text}`;
}
...
(window as any).renderCurrent = async () => {
  ...
  if (!latex) {
    preview.innerHTML = `
      <div class="flex flex-col items-center justify-center h-full">
        <div class="text-red-600 bg-red-50 p-4 rounded-lg border-l-4 border-red-500 shadow-sm w-full">
          <div class="flex items-center gap-2">
            <span class="text-xl">⚠️</span>
            <span class="font-medium">请输入 LaTeX 代码</span>
          </div>
        </div>
      </div>
    `;
  ...
  preview.innerHTML = `
    <div class="flex flex-col items-center justify-center h-full text-gray-500">
      ...
      <div class="text-center italic">正在渲染...</div>
    </div>
  `;
      ...
      <div class="flex flex-col items-center justify-center h-full">
        <div class="text-red-600 bg-red-50 p-4 rounded-lg border-l-4 border-red-500 shadow-sm w-full">
          <div class="flex items-start gap-2">
            ...
            <div>
              <div class="font-medium mb-1">渲染失败</div>
              <div class="text-sm opacity-80">${errorMsg}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  if (preview) {
    preview.innerHTML = `
      <div class="flex flex-col items-center justify-center h-full text-gray-500">
        ...
        <div class="text-center italic">预览已清空</div>
      </div>
    `;
